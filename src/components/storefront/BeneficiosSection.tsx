...
const BENEFICIOS: Beneficio[] = [
  ...
  { icon: "users", titulo: "Atención personalizada", descripcion: "Te ayudamos a elegir el equipo ideal según tu presupuesto." },
];
  ...
  return (
    ...
    >
      <div className="mb-12 text-center">
        ...
        <p className="mx-auto mt-4 max-w-[600px] text-[16px] text-fog-white/70 sm:text-[18px]">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {BENEFICIOS.map((b, i) => (
          <article
            key={b.titulo}
            className={`rounded-cards border border-fog-white/10 bg-navy-surface/40 p-6 transition-all duration-200 hover:-translate-y-1 hover:border-glow-cyan/30 hover:shadow-cyan-glow sm:p-8 ${visible ? "animate-fade-up" : "opacity-0"}`}
            style={{ animationDelay: `${i * 100}ms` }}
          >
            <span className="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-glow-cyan/10 text-glow-cyan">
              <Icon name={b.icon} size={22} />
            </span>
            <h3 className="mt-4 font-sora text-[16px] font-semibold tracking-[-0.01em] text-fog-white sm:text-[18px]">
              {b.titulo}
            </h3>
            <p className="mt-2 text-[13px] leading-relaxed text-fog-white/60 sm:text-[14px]">
              {b.descripcion}
            </p>
          </article>
        ))}
