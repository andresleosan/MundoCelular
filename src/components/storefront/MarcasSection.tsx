...
export function MarcasSection({ marcas }: { marcas: MarcaResumen[] }) {
  ...
  return (
    ...
    >
      ...
      <ScrollReveal className="scroll-reveal-container grid grid-cols-2 items-center gap-3 sm:grid-cols-3 lg:grid-cols-6 lg:gap-4">
        {marcas.map((marca, i) => {
          const className = "scroll-reveal-item group relative flex aspect-[16/9] flex-col items-center justify-center overflow-hidden rounded-cards border border-fog-white/10 bg-navy-surface/40 transition-all duration-300 hover:-translate-y-0.5 hover:border-glow-cyan/30 hover:shadow-cyan-glow focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-2 focus-visible:outline-glow-cyan/40";
          const content = (
            <>
